...
poly1 = kf.kdb.DPolygon(
    [kf.kdb.DPoint(-8,-6),kf.kdb.DPoint(6,8),kf.kdb.DPoint(7,17), kf.kdb.DPoint(9,5)]
)
...
text1 = t.transformed(kf.kdb.DTrans(0.0, 10.0).to_itype(c.klib.dbu)) # DTrans is a transformation in microns with arguments (<rotation in 90° increments>, <mirror boolean>, <x in microns>, <y in microns>)
...
#     
text2 = t.transformed(kf.kdb.CplxTrans(2.0, 45.0, False, 5., 30.))
# text1.movey(25)
# text2.move([5, 30])
# text2.rotate(45)
r.move(-5,0) # boxes can be moved like this, other shapes and cellss/refs need to be moved with .transform
r.move(-5,0)
...
def straight(length=10, width=1, layer=(1, 0)):
    ...
    wg.add_port(kf.DPort(
        name="o1", position=(box.left, box.center().y), width=width, angle=2, layer=_layer)
    )
    wg.create_port(
        name="o2", trans=kf.kdb.Trans(int_box.right, int_box.center().y), layer=_layer, width=int_box.height()
    )
    # wg.draw_ports()
    return wg
...
c2.shapes(c2.klib.layer(10,0)).insert(kf.kdb.Text(
...
e2 = kf.kdb.DPolygon.ellipse(kf.kdb.DBox(10,6),64).transformed(kf.kdb.DTrans(2.,0.))
...
# e1 OR e2 (merged
c = kf.KCell()
e3 = (kf.kdb.Region(e1.to_itype(c.klib.dbu)) + kf.kdb.Region(e2.to_itype(c.klib.dbu))).merge()
c.shapes(c.klib.layer(1,0)).insert(e3)
c
