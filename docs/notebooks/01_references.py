...
#     r2.chain(port="o1", destination=r1.ports["o2"])
...
# ## chain references
#
# We have seen that once you create a reference you can manipulate the reference to move it to a location. Here we are going to chain that reference to a port. Remember that we follow that a certain reference `source` chains to a `destination` port
...
# +
# c = gf.Cell("sample_reference_chain")

# mmi = c << gf.cells.mmi1x2()
# b = c << gf.cells.bend_circular()
# b.chain("o1", destination=mmi.ports["o2"])
...
# +
# c = gf.Cell("sample_reference_chain_simpler")

# mmi = c << gf.cells.mmi1x2()
# b = c << gf.cells.bend_circular()
# b.chain("o1", destination=mmi["o2"])
...
# ## cell_sequence
#
# When you have repetitive chainions you can describe the connectivity as an ASCII map
