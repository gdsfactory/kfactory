...
#     r2.transport(port="o1", destination=r1.ports["o2"])
...
#
# We have seen that once you create a reference you can manipulate the reference to move it to a location. Here we are going to transport that reference to a port. Remember that we follow that a certain reference `source` transports to a `destination` port
...
# +
# c = gf.Cell("sample_reference_transport")

# mmi = c << gf.cells.mmi1x2()
# b = c << gf.cells.bend_circular()
# b.transport("o1", destination=mmi.ports["o2"])
...
# +
# c = gf.Cell("sample_reference_transport_simpler")

# mmi = c << gf.cells.mmi1x2()
# b = c << gf.cells.bend_circular()
# b.transport("o1", destination=mmi["o2"])
...
# ## cell_sequence
#
# When you have repetitive transportions you can describe the connectivity as an ASCII map
