...
def __getattr__(name: str) -> "KLib":
    # TODO: Remove with 0.4.2
    if name != "library":
        raise AttributeError(f"module '{__name__}' has no attribute '{name}'")
    logger.opt(ansi=True).bind(with_backtrace=True).warning(
        "<red>DeprecationWarning:</red> library has been renamed to klib since version 0.3.1 and library will be removed with 0.4.2, update your code to use klib instead",
    )
    return klib
