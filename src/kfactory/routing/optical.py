        ...
        if len(pts) > 2:
            if (vec := pts[1] - pts[0]).abs() == b180r:
                match (p1.trans.angle - vec_angle(vec)) % 4:
                    case 1:
                        ...
                        bend180.chain(b180p1.name, p1)
                        start_port = bend180.ports[b180p2.name]
                        pts = pts[1:]
                    case 3:
                        bend180 = c << bend180_cell
                        bend180.chain(b180p2.name, p1)
            ...
            if (vec := pts[-1] - pts[-2]).abs() == b180r:
                match ((vec_angle(vec) - p2.trans.angle) % 4):
                    case 1:
                        bend180 = c << bend180_cell
                        bend180.chain(b180p1.name, p2)
                        end_port = bend180.ports[b180p2.name]
                        pts = pts[:-1]
                    case 3:
                        bend180 = c << bend180_cell
                        # bend180.mirror = True
                        bend180.chain(b180p2.name, p2)
                ...
                for i in range(3, len(pts) - 2):
                    ...
                    if vecp == vec and ang2 - ang1 == 0:
                        ...
                        if start_port.name == b180p2.name:
                            bend180.chain(b180p1.name, start_port)
                            start_port = bend180.ports[b180p2.name]
                        else:
                            bend180.chain(b180p2.name, start_port)
                            start_port = bend180.ports[b180p1.name]
                        j = i - 1
                    elif (
                        vec.abs() == b180r
...
def place90(
    ...
    if len(pts) == 2:
        ...
        if (
            ...
            if length - (taperp1.trans.disp - taperp2.trans.disp).abs() * 2 != 0:
                ...
                wg_p1, wg_p2 = (v for v in wg.ports if v.port_type == port_type)
                wg.chain(wg_p1, t1, taperp2.name)
                t2 = c << taper_cell
                t2.chain(taperp2.name, wg_p2)
            else:
                t2 = c << taper_cell
                t2.chain(taperp2.name, t1, taperp2.name)
        return
    for i in range(1, len(pts) - 1):
        ...
        if length > 0:
            if (
                ...
                wg_p1, wg_p2 = (v for v in wg.ports if v.port_type == port_type)
                wg.chain(wg_p1, bend90, b90p1.name)
            else:
                t1 = c << taper_cell
                t1.chain(taperp1.name, bend90, b90p1.name)
                if length - (taperp1.trans.disp - taperp2.trans.disp).abs() * 2 != 0:
                    wg = c << straight_factory(
                        width=taperp2.width,
                        length=length
                    ...
                    wg_p1, wg_p2 = (v for v in wg.ports if v.port_type == port_type)
                    wg.chain(wg_p1.name, t1, taperp2.name)
                    t2 = c << taper_cell
                    t2.chain(taperp2.name, wg, wg_p2.name)
                else:
                    t2 = c << taper_cell
                    t2.chain(taperp2.name, t1, taperp2.name)
        old_pt = pt
    ...
    if length > 0:
        if (
            ...
            wg = c << straight_factory(width=w, length=length)
            wg_p1, wg_p2 = (v for v in wg.ports if v.port_type == port_type)
            wg.chain(wg_p1.name, bend90, b90p2.name)
        else:
            t1 = c << taper_cell
            t1.chain(taperp1.name, bend90, b90p2.name)
            if length - (taperp1.trans.disp - taperp2.trans.disp).abs() * 2 != 0:
                wg = c << straight_factory(
                    width=taperp2.width,
                    length=length - (taperp1.trans.disp - taperp2.trans.disp).abs() * 2,
