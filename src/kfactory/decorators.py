...
def _parse_params(
    ...
    """Parse call arguments into a params dict using pre-computed metadata."""
...
class WrappedKCellFunc(Generic[KCellParams, KC]):
    ...
    def __init__(
        ...
        *,
        ...
        _param_defaults, _param_names, _all_param_units = _precompute_sig_metadata(
            sig
        )

        # Define wrapped_cell once with @cached applied once, rather than
        # re-creating it on every call to wrapper_autocell.
        @cached(cache=cache, lock=RLock())
        @functools.wraps(f)
        def wrapped_cell(**params: Any) -> KC:
            # Reconstruct param_units from pre-computed metadata
            param_units = {
        ...
        def wrapper_autocell(
            *args: KCellParams.args, **kwargs: KCellParams.kwargs
            ...
            params = _parse_params(
                _param_defaults, _param_names, kcl, args, kwargs
            )
