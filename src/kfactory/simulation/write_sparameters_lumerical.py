...
def plot_sparameters_lumerical(
    ...
    **settings,
    ...
    if len(s_params) > 1:
        ...
        s.exportcsvresults(paths.as_posix().replace(".npz", ".csv"))

        return paths.as_posix().replace(".npz", "/ONA.csv")
    ...
    if len(s_params) > 1:
        ...
        for value in components.values():
            ...
            if len(instances) == 2:
                ...
                ONA_ports.append([instances[0], ports[0]])
        for i in range(len(component.ports.get_all()) - 1):
            s.connect(
                "ONA",
                f"input {i + 1}",
                f"{ONA_ports[i][0].cell.name, ONA_ports[i][0].hash()}",
                f"{ONA_ports[i][1]}",
            )
        paths = get_sparameters_path(
            component, dirpath=dirpath, simulation_settings=simulation_settings
        ...
        s.connect(
            "ONA",
            "output",
            f"{ONA_ports[-1][0].cell.name, ONA_ports[-1][0].hash()}",
            f"{ONA_ports[-1][1]}",
        )
        s.run()
        s.save(paths.as_posix().replace(".npz", ".ice"))
        ...
        for i, port in enumerate(component.ports.get_all().values()):
            ...
            zmin = layer_to_zmin[_ACTIVE_PDK.get_layer(port.layer)]
            thickness = layer_to_thickness[_ACTIVE_PDK.get_layer(port.layer)]
            z = (zmin + thickness) / 2
        ...
        if run and solver == "FDTD":
            ...
            with open(filepath, "r+") as fd:
                ...
                for p in component.ports.get_all().values():
                    data = data.replace(
                        f"{p.name}, LEFT", f"{p.name}, {diction[p.orientation]}"
                    )
                fd.seek(0)
                fd.write(data)
            fd.close()
