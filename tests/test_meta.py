...
def test_metainfo_dup(straight: kf.KCell) -> None:
    """Test autogenerated port metadata."""

    c_ = straight.dup()
    assert c_.settings == straight.settings
    assert c_.settings_units == straight.settings_units
    assert c_.function_name != straight.function_name
    assert c_.ports == straight.ports
