...
docs python_version="3.14": docs-build-source
    uv run -p {{python_version}} --with-editable . --extra docs --with "{{MIKE}}" --isolated zensical build -f docs/zensical.yml

# Serve documentation locally (zensical) from the pre-built source
docs-serve python_version="3.14": docs-build-source
    uv run -p {{python_version}} --with-editable . --extra docs --with "{{MIKE}}" --isolated zensical serve -f docs/zensical.yml
...
# Copy aliases include objects.inv and assets, which HTML redirects cannot serve.
